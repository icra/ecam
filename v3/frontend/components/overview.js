let overview = new Vue({
  ...
  template:`
    <div id=overview v-if="visible && Languages.ready">
      <div id=overview_tabs>
        <div :current="current_item=='sankey'"    @click="current_item='sankey'">Sankey diagram (GHG)</div>
        <div :current="current_item=='pie_chart'" @click="current_item='pie_chart'">Pie chart (GHG)</div>
        <div :current="current_item=='diagram'"   @click="current_item='diagram'">Diagram stages (water and wastewater flows)</div>
      </div>

      <!--sankey ghg-->
      <div v-if="current_item=='sankey'" style="text-align:center;">
        <h3> GHG emissions sankey diagram </h3>
        <div id=sankey_ghg></div>
      </div>

      <!--charts-->
      <div v-show="current_item=='pie_chart'" style="text-align:center;">
      ...
      <!--diagram stages-->
      <div v-if="current_item=='diagram'" style=text-align:center>
  ...
  style:`
    <style>
      ...
      #overview #overview_tabs > div {
        padding:0 1em;
        border-bottom:4px solid transparent;
        cursor:pointer;
      }
      #overview #overview_tabs > div[current],
      #overview #overview_tabs > div:hover {
...
//draw charts
  function draw_sankey_ghg() {
    ...
    json.links=[
      ...
      {source:get_index('wwt'), target:get_index('fuel'), value:Global.Waste.Treatment.map(s=>s.wwt_KPI_GHG_reus_trck().total).sum()||1},
      {source:get_index('wwt'), target:get_index('disc'), value:Global.Waste.Treatment.map(s=>s.wwt_KPI_GHG_disc()     .total).sum()||1},
